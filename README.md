...
- **What it records:** `facts` citing the numbered snippets they came from (file and line copied mechanically), `assumptions` with the reason, `open_questions` for what the store could not answer, and the derived search `queries` — `asa search` runs only with a recorded query. The proposal prints the facts with their source and the assumptions; an unknown critical attribute means the item is left out with a one-line note — never a round of questions.
- **Privacy:** the brief is local and git-ignored; a vault is read only inside an allow-list of folders, health / finance / mail / relationship / chat-archive folders and index notes are excluded by default, secret notes and `secrets/` by hard-coded rules, and a note opts out with `asa_context: no`; every snippet passes the same redaction filter as the audit log and is dropped when it looks like PII or a secret; audit events carry counts and ids, never note text; nothing is uploaded.
