...
`HUMAN_CONFIRM`, `REF_FULL_NAME`, `REF_DELIVERY_ADDRESS`, `REF_PICKUP_POINT`, and for the Smart! basket
`SMART_THRESHOLD_PLN` (49.90), `SMART_SLACK_PLN` (25), `MAX_COMPLEMENTS` (1), `REORDER_COOLDOWN_DAYS` (30),
`SHOPPING_PROFILE_DIR` (`<private>/shopping-profile`), `DEFAULT_RAIL`, `ALLEGRO_LOGIN`. `config.env` is gitignored.
...
user's own notes and read by `basket:plan`; `profile:check` refuses files that carry address-like data.
...
asa basket:plan [--slack N] [--max-complements N]                 # Smart! basket: seller-grouped plan + complements, one message
