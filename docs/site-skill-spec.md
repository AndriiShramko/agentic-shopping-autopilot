...
Context for 2026: Agent Skills (SKILL.md) became an open, cross-vendor standard. Anthropic released the format as an open standard, reported as 18 December 2025 (itecsonline.com) [reported]; support is declared by 46 platforms listed in the client showcase at https://agentskills.io (accessed 2026-09-03), including Claude Code, OpenAI Codex, GitHub Copilot, Cursor, Gemini CLI and VS Code [fact: agentskills.io, accessed 2026-09-03]. Codex CLI loads skills from `~/.agents/skills/` (and from `.agents/skills` inside the repository) and names the open Agent Skills standard as its basis [fact: OpenAI Codex skills documentation]. This removes the main historical risk of the format: vendor lock-in on a single agent.
