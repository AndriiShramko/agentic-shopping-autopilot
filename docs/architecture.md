...
**2026-09-05 — first live purchase completed end-to-end.** Channel A (the user's own Chrome with the Claude in Chrome extension, logged-in profile): cart → delivery & payment → the default payment method was a bank redirect ("Przelew"), switched explicitly to a saved one-click card in the "Wybierz kartę płatniczą" modal → "Kupuję i płacę" → "Zakup opłacony" in about 10 s, no bank challenge for an order under 50 zł; confirmed on the purchases page. Lessons: the extension does complete purchases (an earlier note claiming a policy block was wrong, so the dedicated CDP profile is not needed); the checkout model gains a "select saved card" step; Allegro Pay may appear only as an inactive "Business" variant on accounts that invoice to a company, leaving the saved card as the only rail.
